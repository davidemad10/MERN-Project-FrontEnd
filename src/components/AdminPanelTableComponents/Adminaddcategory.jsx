...
function AddCategory() {
  ...
  function handleCategory(e) {
    ...
    fetch("https://goodreadfdm.vercel.app/categories", {
      method: "POST",
      headers: {
      ...
      .then((data) => {
        ...
        if (
          data.errorMessage &&
          data.errorMessage.includes("the categoryName must be at least 3 char")
        ) {
  ...
  return (
    <div className="shadow rounded p-4 position-absolute w-50 top-50 start-50 translate-middle addcategoryCard bg-light">
      ...
      <form onSubmit={handleCategory}>
        ...
        />
        <div>
          <button className="btn btn-success mt-4 px-5" type="submit">
            Add Category
          </button>
          <button
            className="btn btn-dark ms-2 mt-4 px-5"
